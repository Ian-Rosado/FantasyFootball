...
- `scripts/sleeper_defense_streaming.py` — **in-season** D/ST streamer for the separate **DyNasty (Sleeper)** dynasty league (`1358499663992348672`, 10-team 1QB .5PPR, 2 FLEX). Availability comes from the Sleeper public read API (a DEF's `player_id` IS its team code, so available = 32 teams minus every rostered DEF id — no big `/players/nfl` dump needed); matchups come from ESPN's public CDN scoreboard (`cdn.espn.com/core/nfl/scoreboard?xhr=1`, since Sleeper doesn't expose the NFL schedule; `site.api.espn.com` is WAF-blocked from this env). Note ESPN abbreviations differ (WSH/JAC/LVR) and are normalized to Sleeper's (WAS/JAX/LV). Unlike Gridiron Grind, this league scores points/yards allowed heavily, so it targets good D's facing WEAK/low-scoring offenses. `python sleeper_defense_streaming.py`.
