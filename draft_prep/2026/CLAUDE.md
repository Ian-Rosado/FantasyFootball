...
- `scripts/defense_streaming.py` — **in-season, multi-week** D/ST streamer: availability = 32 NFL defenses minus every one already rostered (one Fleaflicker `FetchLeagueRosters` call — no more paginating the FA pool, which tripped the API rate limit); the live schedule (this week + `--ahead` N, pinned via `--week`) comes from ESPN's public CDN scoreboard. Prints a matchup grid + a GRAB-AHEAD section (available now, good matchup in a later week), flagging sack/turnover-prone opponents via the hand-maintained `ATTACK_OFFENSES` list (this league scores no points/yards allowed — stream for sacks + takeaways + defensive TDs; the schedule is live, the opponent flag is a tilt to refresh). `python defense_streaming.py [--week N] [--ahead 2]`.
- `scripts/waiver_bids.py` and `defense_streaming.py` use the public Fleaflicker API (std-lib only) and can rate-limit on rapid repeat runs — wait a minute if you get the "API unavailable" message.
- `scripts/sleeper_defense_streaming.py` — **in-season, multi-week** D/ST streamer for the separate **DyNasty (Sleeper)** dynasty league (`1358499663992348672`, 10-team 1QB .5PPR, 2 FLEX). Availability comes from the Sleeper public read API (a DEF's `player_id` IS its team code, so available = 32 teams minus every rostered DEF id — no big `/players/nfl` dump needed); matchups (this week + `--ahead` N, pinned via `--week`) come from ESPN's public CDN scoreboard (`cdn.espn.com/core/nfl/scoreboard?xhr=1`, since Sleeper doesn't expose the NFL schedule; `site.api.espn.com` is WAF-blocked from this env). Note ESPN abbreviations differ (WSH/JAC/LVR) and are normalized to Sleeper's (WAS/JAX/LV). Prints an availability grid, **your own** rostered defenses' upcoming schedule (via `MY_USER`), and a GRAB-AHEAD section. Unlike Gridiron Grind, this league scores points/yards allowed heavily, so it targets good D's facing WEAK/low-scoring offenses (hand-maintained `TARGETS`). `python sleeper_defense_streaming.py [--week N] [--ahead 2]`.
- `scripts/sleeper_waiver.py` — **in-season** waiver helper for DyNasty (Sleeper): generalizes the roster-diff trick to all positions (owned = union of every roster's players; available = the `/players/nfl` pool minus owned, cached ~daily in the OS temp dir), ranks the best available skill players by Sleeper's leaguewide trending-add signal, and prints Team L.O.B's roster by position (flagging spots below a minimum depth). `python sleeper_waiver.py [--pos RB] [--limit 25]`.
- `scripts/espn_defense_streaming.py` — **in-season, multi-week** D/ST streamer for the separate **Eat GLORP (ESPN)** league (`686982204`, team "Eat GLORP", 10-team). Same roster-diff approach: availability = 32 NFL defenses minus every one rostered (ESPN `mRoster` view); matchups (this week + `--ahead` N, pinned via `--week`) + **your own** rostered defenses' schedule + a GRAB-AHEAD section come from ESPN's CDN scoreboard. This league scores points/yards allowed heavily, so it targets good D's facing WEAK/low-scoring offenses (hand-maintained `TARGETS`). ESPN proTeamId→abbrev via the `PROTEAM` map. `python espn_defense_streaming.py [--week N] [--ahead 2]`.
- `scripts/espn_waiver.py` — **in-season** waiver helper for Eat GLORP (ESPN): prints your roster by position (flagging thin spots) then the best available free agents by **% rostered** (with weekly trend), via ESPN's `kona_player_info` view + an `X-Fantasy-Filter` header for FREEAGENT/WAIVERS status. `python espn_waiver.py [--pos RB] [--limit 25]`.
- **ESPN private-league auth:** Eat GLORP is a *private* league, so both `espn_*` scripts need your `espn_s2` + `SWID` cookies. They load from `ESPN_S2`/`ESPN_SWID` env vars or a local `scripts/.espn_cookies.json` (copy `.espn_cookies.example.json`). That file is **git-ignored** (`scripts/.gitignore`) — never commit it. Cookies expire; on a 401 the scripts tell you to refresh them from the browser. ESPN `lm-api-reads.fantasy.espn.com` read endpoints work with these; the CDN scoreboard needs no auth.
