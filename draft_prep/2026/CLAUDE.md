...
- **Competition panel (`renderComp`):** per position — teams still needing it, projected clearing price of best player left (`fair × inflation × posDemand`), how many of YOUR plan-tier remain affordable (+ count in next tier down), and a "behind?" read.
